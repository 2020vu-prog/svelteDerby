...
class OpusFileStream {
    ...
    parseOpusTOC(data) {
        ...
        if (tocC === 0) {
            ...
            console.log(
                "An arbitrary number of frames in the packet - possible audio artifacts"
            );
            framesPerPacket = 1;
        }
        ...
        if (this.segmentIndex < this.segmentsCount) {
            ...
            this.getNextOggPacketStart((success) => {
                ...
                this.parseOggPacketHeader((success) => {
                    ...
                    if (
                        isContinueNeeded &&
                        lastSequenceNum + 1 !== this.sequenceNumber
                    ) {
                        this.segmentsCount = -1;
                        console.log(
                            "Skipping frame: continuation sequence is broken"
                        );
                        return this.getNextOpusPacket(
                            null,
                            false,
                            onCompleteCb,
                            allHeadersParsed
            ...
            (segmentData, isContinueNeeded) => {
                ...
                if (packet) {
                    ...
                    let newdata = new Uint8Array(
                        packet.length + segmentData.length
                    );
                    newdata.set(packet);
                ...
                if (
                    this.framesPerPacket !== frames ||
                    this.packetDurationMs !== duration
                ) {
                    packet = null;
