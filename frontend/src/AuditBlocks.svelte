<script>
    ...
    function recalcMessages(carNumberForm, $doRefreshBlocks) {
        ...
        msgs.push({
            msg: `pt: ${carNumberForm.promptPhaseType} needed: ${neededForType}`,
            level: log.levels.DEBUG,
        });

        msgs.push({
            msg: `Pending len: ${getPendings().length}`,
            level: log.levels.DEBUG,
        });

        if (!c1 && !c2) {
            msgs.push({
                msg: `At least 1 car needed.`,
        ...
        if (pendingRs) {
            if (neededForType) {
                if (lanesOK(pendingRs)) {
                    msgs.push({
                        msg: `Pending found for cars.`,
                        level: log.levels.INFO,
                    });
                } else {
                    msgs.push({
                        msg: `Pending found for cars. Lanes reversed?`,
                        level: log.levels.ERROR,
                    });
                }
            } else {
                msgs.push({
                    msg: `Pending found. Should this be a race?`,
    ...
    function twoCarsPending() {
        const p = getPendings();
        return p.find((rs) =>
            isCarPending(rs, carNumberForm.car1, carNumberForm.car2)
        );
    }
