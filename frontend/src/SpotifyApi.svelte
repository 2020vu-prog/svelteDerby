...
<script lang="ts">
    ...
    import {spotifyActiveDeviceId, spotifyLookupTrack, spotifyPlay} from './utils/spotify.js'
    ...
    const requestPlay = async () => {
        ...
        try {
            ...
            if (!destroyed) {
                setWalkupStatus(`Spotify walk-up failed: track lookup failed: ${error.message}`);
            }
            return false;
        }
        ...
        setWalkupStatus(`Spotify walk-up play requested: “${trackName}”.`, "success");
        const targetDeviceId = await getWalkupDevice();
        if (destroyed) return false;
        if (!targetDeviceId) {
            setWalkupStatus(`Spotify walk-up failed for “${trackName}”: no playback device is available.`);
            return false;
        }
        ...
        if (destroyed) return false;
        if (!result?.ok) {
            setWalkupStatus(`Spotify walk-up failed for “${trackName}”: ${result?.reason || "unknown playback error."}`);
            return false;
        }
        setWalkupStatus(`Spotify walk-up playback started: “${result.track?.name || trackName}”.`, "success");
        return true;
    };
