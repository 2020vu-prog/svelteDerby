...
export function recalcLaneData(finishBlocks, cnList) {
    ...
    sortedFB.forEach((fb) => {
        ...
        if (prevFB.timerConfig && fbHasGps(fb)) {
            ...
            fmap.l1[`${fb.timerConfig.seq}_Split_${fb.timerConfig.timerName}`] =
                l1s;
            fmap.l2[`${fb.timerConfig.seq}_Split_${fb.timerConfig.timerName}`] =
                l2s;
        } else {
        ...
        if (fbHasGps(fb)) {
            ...
            fmap.l1[`${fb.timerConfig.seq}_Time_${fb.timerConfig.timerName}`] =
                fmtMsHHMMSS(fb.gpsNoseMs[0]);
            ((fmap.l2[
                `${fb.timerConfig.seq}_Time_${fb.timerConfig.timerName}`
            ] = fmtMsHHMMSS(fb.gpsNoseMs[1])),
                (prevFB = fb));
        } else {
            //no gps
