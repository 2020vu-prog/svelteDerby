...
5. **Timestamp domain**: don't convert `EncodedVideoChunk.timestamp` (encoder-clock microseconds) to wall time. Instead, stamp each chunk with `Date.now()` live inside the encoder's `output` callback — exactly paralleling `growBlob()`'s existing `activeSnipList[snum].snipEnd = Date.now()` (line 753). This avoids introducing a second, incompatible time system; `videoClientTimeAdjustmentMs`/`calcClientTimeAdjustmentMs` (server clock sync) are reused untouched. One caveat to measure, not assume: the `output` callback fires after encode latency, so this is "encoded-and-available" time, not exact capture time. The app already tolerates ±500ms slop in `handleRemoteRequest` and a 300ms pre-roll in `deferredCapture`, so this is likely fine — but should be validated against a real timing reference in the spike below.
...
- **Manual backpressure**: unlike `MediaRecorder`'s opaque internal buffering, WebCodecs requires checking `encoder.encodeQueueSize` before each `encode()` call and dropping/closing frames if the encoder falls behind (e.g. under thermal throttling), plus explicit `.close()` on every `VideoFrame`/`EncodedVideoChunk` once consumed, or the tab leaks GPU/CPU buffers.
