...
- **Frontend automated tests now exist, but only at the pure-JS-logic layer.** [#119](https://github.com/2020vu-prog/svelteDerby/pull/119) added a `node --test` suite — 5 files (`regexEscape`, `paInfo`, `timerColumns`, `routeHelp`, `routeRegistry`), wired to `npm test` in `frontend/package.json` and running in CI (`.github/workflows/format.yml`). None of them render a `.svelte` file or touch `sveltestrap`/`svelte-spa-router` — they cover standalone modules deliberately kept free of Svelte/store/Dexie dependencies so plain Node can run them. **Zero of the 94 `.svelte` components have any test coverage**, and there is no browser/e2e layer. That gap — not "no tests at all" — is what still forces this migration to be verified by hand, and it's exactly the gap Phase 0 below now closes.
...
**Phase 0 — Safety net, before touching any version number.** The existing `node --test` suite covers pure-JS logic only — no `.svelte` component has ever been rendered in a test, and there's no browser layer — so every phase below is still verified by hand unless that gap closes first. Two additions close it, run alongside the existing suite rather than replacing it:

- **Component tests** (`@testing-library/svelte` + Vitest, since `node --test` can't compile `.svelte` files the way the existing `.mjs` suite assumes). At minimum, one smoke test per risk pattern this migration actually touches: a component using a `sveltestrap` component (e.g. `Modal`), one calling `svelte-spa-router`'s `push`/`pop`/`replace`, and one using `createEventDispatcher`. That gives Phase 1's diagnostic spike and Phase 2's dependency swaps something to run instead of a visual pass.
- **E2E smoke tests** (Playwright) against a running dev build, covering the highest-stakes flows: login, event selection, race timing/announcing, and the walkup-track playback path this session's own recent work touched. Higher-leverage than the component tests for this migration specifically, since it exercises router + sveltestrap + real user flows together in one pass.

Do all of this work on a long-lived branch, and use the existing `stage.rr1.us`/`go.rr1.us` branches as a real soak environment before anything reaches `test.rr1.us` — this app runs live in-person events, so "looks fine in a local dev build" isn't sufficient confidence on its own. Note the Vitest/`@testing-library/svelte`/`@sveltejs/vite-plugin-svelte` versions Phase 0 lands with are necessarily pinned to what supports Svelte 3 today; they'll need their own version bump in lockstep with Phase 3's `svelte` bump, the same way `sveltestrap` and `svelte-spa-router` do.
...
- Given zero frontend *component or e2e* coverage today (pure-JS logic is covered, `.svelte` rendering and real user flows are not), how much of Phase 0's investment happens before this project starts versus alongside it — this is a real scoping/timeline decision, not just a nice-to-have footnote.
